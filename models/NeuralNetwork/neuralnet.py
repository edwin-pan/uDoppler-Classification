...
class NeuralNet(torch.nn.Module):
    def __init__(self, lrate,loss_fn,in_size,out_size):
        ...
        self.CNN_layer1 = torch.nn.Sequential(torch.nn.Conv2d(3,32,5,stride=1, padding=2),
                                        torch.nn.LeakyReLU(),
        ...
        # good one 8*8*128
        self.model = torch.nn.Sequential(torch.nn.Linear(8*8*128, 96),
...
def fit(train_set,train_labels,dev_set,n_iter,batch_size=100):
    ...
    ReturnsL
        ...
        yhats: an (M,) NumPy array of approximations to labels for dev_set
    ...
    dev_set = Normalize(dev_set,sample=False)
    ...
    # Reshape to 7500,3,32,32
    train_set_img = train_set.view((-1,3,32,32))
    ...
    # Evaluate
    net.eval()
    yhats_raw = net(dev_set).detach().numpy()
